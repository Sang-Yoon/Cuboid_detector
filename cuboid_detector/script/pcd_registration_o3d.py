...
class PointCloudRegistration:
    ...
    def __init__(self) -> None:
        ...
        self.LOCAL_REGISTRATION = False
        self.CUDA = False
        self.RANSAC = False
        self.FGR = False
        self.VANILLA_ICP = False
        self.MULTI_SCALE_ICP = False
    ...
    def remove_hidden_points(self, pcd):
        ...
        radius = diameter * 5000  # The radius of the sperical projection
    ...
    def draw_registration_result(self, src, dst, result, window_name="After registration"):
        ...
        transformation = result.transformation if result is not None else np.identity(4)
        ...
        if result is not None:
            ...
            if isinstance(result, o3d.cuda.pybind.t.pipelines.registration.RegistrationResult):
                corr = result.correspondences_.cpu().numpy()
                corr = [(int(i), corr[i]) for i in range(len(corr))]
                corr = [(i, j) for i, j in corr if i % 20 == 0]
            else:
...
def main():
    ...
    pcd_path = os.path.join(os.path.dirname(__file__), "pvn3d/datasets/ycb/YCB_Video_Dataset/models/t_pipe/points.xyz")
    ...
    init_src_temp_to_target = result_global_refinement.transformation if pcd_reg.GLOBAL_REGISTRATION else np.identity(4)
    ...
    if pcd_reg.LOCAL_REGISTRATION:
        ...
        if pcd_reg.VANILLA_ICP:
            if pcd_reg.CUDA:
                ...
                result_local_refinement, _ = pcd_reg.execute_ICP_registration_CUDA(
                ...
                pcd_reg.draw_registration_result(
                    pcd_src_cuda.to_legacy(), pcd_dst_cuda.to_legacy(), result_local_refinement
                )
            else:
                result_local_refinement, _ = pcd_reg.execute_ICP_registration(src, dst, voxel_size, init_src_temp_to_target)
                print("Local registration took %.3f sec.\n" % (time.time() - start_local_registration))
                pcd_reg.draw_registration_result(src, dst, result_local_refinement)
